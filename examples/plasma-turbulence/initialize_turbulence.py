## read plasma configuration from file
...
class Configuration_Turbulence(Configuration):
    ...
    def __init__(self, *file_names, do_print=False):
        ...
        self.binit_nc = sqrt(ppc * c**2.0 * self.sigma * me)
        ...
        self.warm_corr = 0.5 * (
            (1.0 + corrdelgam_sig * gad_i / (gad_i - 1.0) * self.delgam_i)
            + (1.0 + corrdelgam_sig * gad_e / (gad_e - 1.0) * self.delgam_e)
        )

        self.binit_warm = sqrt(
            ppc
            * 0.5
            * c**2.0
            * (
                mi * (1.0 + corrdelgam_sig * gad_i / (gad_i - 1.0) * self.delgam_i)
                + me * (1.0 + corrdelgam_sig * gad_e / (gad_e - 1.0) * self.delgam_e)
            )
            * self.sigma
        )
        ...
        self.binit_approx = sqrt(self.gammath * ppc * me * c**2.0 * self.sigma)

        # manual value for theta=0.3
        # self.gammath = 1.55

        self.gammath = 1.0  # cool plasma
        self.binit = sqrt(self.gammath * ppc * me * c**2.0 * self.sigma)

        self.sigma_perp = self.sigma * self.drive_ampl**2
        self.binit_perp = sqrt(self.gammath * ppc * me * c**2.0 * self.sigma_perp)
        ...
        if "gammarad" in self.__dict__:
            if not (self.gammarad == 0.0):
                ...
                if do_print:
                    ...
                    print(
                        "init:  drag amplitude: {} with gamma_rad: {}".format(
                            self.drag_amplitude, self.gammarad
                        )
                    )
        else:
            self.gammarad = 0.0
        ...
        lsig = (
            self.sigma_perp / np.sqrt(self.sigma_perp) * np.sqrt(self.gammath)
        )  # gamma_0
