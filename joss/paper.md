...
Magnetized plasma turbulence is a ubiquitous and complex mechanism in high-energy astrophysics and space plasma physics. In a classical Kolmogorov turbulence flow [@kolmogorov1941], kinetic energy is **perfectly** transferred from a large scale $l_0$ (i.e. the driving scale) down to smaller scales, $l_0 > l_1 > l_2$ ..., where the resulting cascade is self-similar in nature. In a magnetized plasma, however, non-linear interactions between the charged particles and the external magnetic field give rise to *intermittency*--spatio-temporal fluctuations in the turbulent flow. So far, numerical simulations and experiments have demonstrated the manifestation of such intermittency in the form of *current sheets*. These current sheets arise via interactions between magnetic coils within the flow, and are important in regulating the dissipation of magnetic energy via magnetic reconnection [@priest2000] and (non-thermal) particle acceleration [@lemoine2023]. 

Until recently, detection and statistically analysis of these intermittent structures have been done mostly by hand, via a combination of simple thresholds in the current density [@zhdankin2013] and manual verification of magnetic-field reversal across a boundary [@kadowaki2018]. Recent advancements in computational techniques, particularly the rapid development of machine learning methodologies and its application to astrophysical research questions, have highlighted the need for a more robust and efficient approache to understanding the nature of the intermittency in plasma turbulence. To this end, we introduce `aweSOM`, a CPU/GPU-accelerated Python package that extends the capabilities of a generic Self-organizing Map (SOM) model [@hamel2019, @yuan2018] and a Statistically Combined Ensemble (SCE) method [@bussov2021], using `JAX` to significantly improve the performance of the mask-to-mask stacking procedure. Altogether, `aweSOM` provides a fast and robust framework to segment regions of intermittency in 3-dimensional plasma turbulence simulations.
