...
# Use JAX if GPU is available, otherwise use NumPy
try:
...
def compute_SQ(mask : array_lib.ndarray, maskC : array_lib.ndarray):
    ...
    if array_lib.max(mask) == 0  or array_lib.max(maskC) == 0:
        return 0., array_lib.zeros(mask.shape), 0., 0.
    
    Q = array_lib.sum(U) / (array_lib.sum(mask) + array_lib.sum(maskC)) - array_lib.sum(I) / (array_lib.sum(mask) + array_lib.sum(maskC))
    if Q == 0.0:
        return 0., array_lib.zeros(mask.shape) #break here because this causes NaNs that accumulate.
...
def loop_over_all_clusters(all_files: list[str], number_of_clusters: array_lib.ndarray, dimensions : np.ndarray) -> int:
    """
    ...
    for i in range(len(runs)):
        ...
        for cid in range(nids):
            ...
            array_lib.save(subfolder + '/mask-{}-id{}.npy'.format(run.strip('.npy'), cid), total_mask)
